import { getQTableApi } from "./api.js";
import { showSpinner, hideSpinner } from "./spinner.js";

const quickTableContainer = document.querySelector(".categories__table");
let fetchQTable;

export function initQtable(qTableApiUrl) {
  fetchQTable = getQTableApi(qTableApiUrl);
}

export function renderTable(tableHtml) {
  var range = document.createRange();
  var fragment = range.createContextualFragment(tableHtml);
  quickTableContainer.replaceChildren(fragment);
}

export async function getAndRenderQuickTable(requestBody) {
  showSpinner();

  const body = buildRequestBodyFromQTable() + "&" + requestBody;
  const response = await fetchQTable(body);
...
export async function submitform2__override(...args) {
