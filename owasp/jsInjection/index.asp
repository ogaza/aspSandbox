    ...
    <section class="section--main">
      ...
      <div>
        ...
        <form method="post" id=form1 name=form1 onSubmit="" accept-charset="UTF-8">
          ...
          <input type="text">
          <%
          Dim ahID : ahID = "'/> <script language='javascript'>console.log(document.querySelector('#ahID_encoded').value)</script>"
          ' Dim ahID : ahID = "<script language='javascript'>console.log('ahID')</script>"
          %>
          <input type=hidden id=ahID_encoded name="ahID" value='<%=Reform.HtmlAttributeEncode(ahID)%>' readonly>
          
          <!--
          uncommenting the folowing input will result in script injection
          via ahID variable
           -->
          <!--
          <input type=hidden id=ahID name="ahID" value='<%=ahID%>' readonly>
          -->
          ...
          -->
          <input type="text" name="CompanyUrl" value ='<%=company_url%>'>
        </form>
      </div>
    </section>
  </main>
